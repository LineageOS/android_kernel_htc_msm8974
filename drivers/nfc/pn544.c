...
static int pn544_RxData(uint8_t *rxData, int length)
{
	...
	pr_debug("%s: [addr=%x flag=%x len=%x]\n", __func__,
		msg[0].addr, msg[0].flags, msg[0].len);

	if (ignoreI2C) {
		pr_debug("%s: ignore pn544_RxData %d\n", __func__, ignoreI2C);
		return 0;
	}
...
static int pn544_TxData(uint8_t *txData, int length)
{
	...
	if (ignoreI2C) {
		pr_debug("%s: ignore pn544_TxData %d\n", __func__, ignoreI2C);
		return 0;
	}
		...
		size_t count, loff_t *offset)
{
	...
	pr_debug("%s: start count = %u\n", __func__, count);

	if (count > MAX_BUFFER_SIZE) {
		pr_err("%s : count =%zu> MAX_BUFFER_SIZE\n", __func__, count);
		count = MAX_BUFFER_SIZE;
	}

	val = gpio_get_value(pni->irq_gpio);

	pr_debug("%s: reading %zu bytes, irq_gpio = %d\n", __func__, count, val);

	mutex_lock(&pni->read_mutex);

	if (!gpio_get_value(pni->irq_gpio)) {
		if (filp->f_flags & O_NONBLOCK) {
			pr_debug("%s: f_flags & O_NONBLOCK read again\n", __func__);
			ret = -EAGAIN;
			goto fail;
		}
		...
		pr_debug("%s: waiting read-event INT, because irq_gpio = 0\n", __func__);
		pni->isReadBlock = true;
		ret = wait_event_interruptible(pni->read_wq, gpio_get_value(pni->irq_gpio));

		pn544_disable_irq(pni);

		pr_debug("%s: wait_event_interruptible done\n", __func__);

		if (ret) {
			pr_debug("%s: wait_event_interruptible breaked ret=%d\n", __func__, ret);
			goto fail;
		}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 3) {
			pr_info("%s: Software reset\n", __func__);
			if (pn544_TxData(buffer, 6) < 0)
				pr_err("%s: SW-Reset TxData error!\n", __func__);
		} else if (arg == 2) {
			...
			pr_info("%s: power on (delay50)\n", __func__);
			gpio_set_value(pni->firm_gpio, 0);
			pn544_enable();
			msleep(50);
			is_debug = 1;
			s_wdcmd_cnt = 0;
			pr_info("%s: pn544_enable, set is_debug = %d, s_wdcmd_cnt : %d\n",
					__func__, is_debug, s_wdcmd_cnt);
		} else  if (arg == 0) {
			pr_info("%s: power off (delay50)\n", __func__);
			gpio_set_value(pni->firm_gpio, 0);
			pn544_disable();
			msleep(50);
			is_debug = 0;
			pr_info("%s: pn544_disable, set is_debug = %d, s_wdcmd_cnt = %d\n",
					__func__, is_debug, s_wdcmd_cnt);
		} else {
			pr_err("%s: bad arg %lu\n", __func__, arg);
			goto fail;
...
#else
	if (val != 1)
		pr_err("%s: ####### data not ready -> force to read!#########\n", __func__);
				...
				const char *buf, size_t count)
{
	...
	int i2cw_len = 0;
	int code = -1, ret = -1, i = 0;
	char *ptr;

	sscanf(buf, "%d", &code);

	pr_debug("%s: irq = %d,  ven_gpio = %d,  firm_gpio = %d +\n", __func__,
	...
	case 6:
		...
		if (ptr != NULL) {
			...
			pr_debug("%s: skip no data found\n", __func__);
		break;
	case 7:
		pr_debug("%s: case 7 disable i2c\n", __func__);
		ignoreI2C = 1;
		break;
	case 8:
		pr_debug("%s: case 8 enable i2c\n", __func__);
		ignoreI2C = 0;
		break;
	default:
		pr_err("%s: case default\n", __func__);
		break;
	}
...
{
	int ret = 0;
	pr_debug("%s not implement yet!! mfc_nfc_cmd_result is %d\n", __func__, mfc_nfc_cmd_result);
	ret = snprintf(buf, MAX_BUFFER_SIZE*2 ,"%d\n", mfc_nfc_cmd_result);
	return ret;
}
				...
				const char *buf, size_t count)
{
	...
	sscanf(buf, "%d", &code);
	pr_debug("%s: not implement yet!! store value = %d\n", __func__, code);

	switch (code) {
	case 0:
		pr_debug("%s: not implement yet!! get nfcversion :\n", __func__);
		mfc_nfc_cmd_result = -100;
		break;
	case 1:
		pr_debug("%s: not implement yet!! nfcreader test :\n", __func__);
		mfc_nfc_cmd_result = -100;
		break;
	case 2:
		pr_debug("%s: not implement yet!! nfccard test :\n", __func__);
		mfc_nfc_cmd_result = -100;
		break;
	default:
		pr_err("%s: not implement yet!! case default do nothing\n", __func__);
		break;
	}
	pr_debug("%s: END\n", __func__);
	return count;
...
static int pn544_parse_dt(struct device *dev, struct pn544_i2c_platform_data *pdata)
{
	...
	if (prop) {
		...
		pr_info("%s: ven_isinvert = %d", __func__, pdata->ven_isinvert);
	}

	prop = of_find_property(dt, "nxp,isalive", NULL);
	if (prop) {
		of_property_read_u32(dt, "nxp,isalive", &is_alive);
		pr_info("%s: is_alive = %d", __func__, is_alive);
	}
	...
	pr_info("%s: End, irq_gpio:%d, ven_gpio:%d, firm_gpio:%d\n",
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (platform_data == NULL) {
			dev_err(&client->dev, "%s: failed because platform_data is NULL\n", __func__);
			return  -ENODEV;
		}
		ret = pn544_parse_dt(&client->dev, platform_data);
		if (ret) {
			dev_err(&client->dev, "%s: pn544_parse_dt fail\n", __func__);
			ret = -ENODEV;
			goto err_exit;
		}
	} else {
	...
	if (ret) {
		dev_err(&client->dev, "%s: request gpio%d fail\n", __func__, platform_data->irq_gpio);
		ret = -ENODEV;
		goto err_exit;
	}
	...
	if (platform_data->gpio_init != NULL) {
		dev_info(&client->dev, "%s: gpio_init\n", __func__);
	...
	pni->isReadBlock = false;
	dev_info(&client->dev, "%s : irq_gpio:%d, ven_gpio:%d, firm_gpio:%d, ven_enable:%d\n",
	...
	dev_info(&client->dev, "%s: init io_wake_lock\n", __func__);
...
#endif
	if (ret) {
		dev_err(&client->dev, "%s: %s: misc_register failed\n", __func__, __FILE__);
	...
	if (ret) {
		dev_err(&client->dev, "%s: device_create_file dev_attr_pn_temp1 failed\n", __func__);
		goto err_create_pn_file;
	}

	ret = device_create_file(pni->pn_dev, &dev_attr_debug_enable);
	if (ret) {
		dev_err(&client->dev, "%s: device_create_file dev_attr_debug_enable failed\n", __func__);
		goto err_create_pn_file;
	}
	...
	if (ret)
		dev_err(&client->dev, "%s: device_create_file dev_attrnxp_uicc_swp failed\n", __func__);

	ret = device_create_file(pni->comn_dev, &dev_attr_mfg_nfc_ctrl);
	if (ret)
		dev_err(&client->dev, "%s: device_create_file dev_attr_mfg_nfc_ctrl failed\n", __func__);

	if (is_alive) {
		if (pni->boot_mode != 5) {
			dev_info(&client->dev, "%s: disable NFC by default (bootmode = %d)\n", __func__, pni->boot_mode);
			pn544_disable();
		}
	}

	if (is_alive == 0) {
		dev_info(&client->dev, "%s: Without NFC, device_create_file dev_attr_nxp_chip_alive \n", __func__);
		ret = device_create_file(pni->pn_dev, &dev_attr_nxp_chip_alive);
		if (ret) {
			dev_err(&client->dev, "%s: device_create_file dev_attr_nxp_chip_alive failed\n", __func__);
			goto err_create_pn_file;
		}
	}

	dev_info(&client->dev, "%s: Probe success! is_alive : %d, is_uicc_swp : %d\n",
...
static int pn544_suspend(struct i2c_client *client, pm_message_t state)
{
	...
	dev_info(&client->dev, "%s: irq = %d, ven_gpio = %d, isEn = %d, isReadBlock =%d\n",
			__func__, gpio_get_value(pni->irq_gpio), gpio_get_value(pni->ven_gpio),
			pn544_isEn(), pni->isReadBlock);
...
{
	pr_info("%s: Loading pn544 driver\n", __func__);
...
{
	pr_info("%s: Unloading pn544 driver\n", __func__);
