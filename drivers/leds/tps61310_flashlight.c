...
static DEVICE_ATTR(poweroff, 0220, NULL, poweroff_store);
...
int tps61310_flashlight_mode(int mode)
{
	...
	pr_debug("camera flash current %d. ver: 0220\n", mode);
	mutex_lock(&tps61310_mutex);
...
int tps61310_flashlight_mode2(int mode2, int mode13)
{
	...
	pr_debug("camera flash current %d+%d. ver: 0220\n", mode2, mode13);
	mutex_lock(&tps61310_mutex);
	if (this_tps61310->reset && reg_init_fail) {
		reg_init_fail = 0;
		if (this_tps61310->enable_FLT_1500mA) {
...
int tps61310_flashlight_control(int mode)
{
	...
	if (rc)
		pr_debug("tps61310_delayed_work is cancelled\n");
				    ...
				    enum led_brightness brightness)
{
	...
	if ((mode != FL_MODE_OFF) && switch_state == 0){
		pr_debug("%s flashlight is disabled by switch, mode = %d\n",__func__, mode);
		return;
	}
...
static int tps61310_parse_dt(struct device *dev,
		...
		struct device_node *node)
{
	...
	rc = of_property_read_string(node, "linux,default-trigger", &temp_string);
	if (!rc)
	...
	rc = of_property_read_string(node, "htc,default-state", &temp_string);
	if (!rc) {
				...
				enum led_brightness value)
{
	...
	if ( value == 0 )
		...
		uint8_t enled = 0x68;
		pr_debug("dualflash 1.5A - simple\n");
		switch (led->id) {
		case LED_2:
				...
				enum led_brightness value)
{
	...
	if ( value == 0 )
		...
		pr_debug("dualtorch 1.5A - simple\n");
		switch (led->id) {
		case LED_2:
...
{
	struct tps61310_led_data *led = container_of(work, struct tps61310_led_data, work);
...
static int tps61310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (board_mfg_mode() == MFG_MODE_OFFMODE_CHARGING) {
		dev_dbg(&client->dev, "offmode_charging, do not probe tps61310_flashlight\n");
		return -EACCES;
	}
	...
	if (tps61310->strb0) {
		...
		if (ret) {
			dev_err(&client->dev, "Unable to request gpio %d (%d)\n",
					tps61310->strb0, ret);
			return ret;
		}
		...
		if (ret) {
			dev_err(&client->dev, "Unable to set direction\n");
			return ret;
		}
	}
	if (tps61310->strb1) {
	...
	for_each_child_of_node(node, temp) {
		...
		if (rc < 0) {
			dev_err(&client->dev, "Failure reading label, rc = %d\n", rc);
			goto fail_id_check;
		}

		rc = of_property_read_string(temp, "linux,name", &led->cdev.name);
		if (rc < 0) {
			dev_err(&client->dev, "Failure reading led name, rc = %d\n", rc);
			goto fail_id_check;
		}
		...
		if (rc < 0) {
			dev_err(&client->dev, "Failure reading led id, rc =  %d\n", rc);
			goto fail_id_check;
		}

		rc = tps61310_get_common_configs(led, temp);
		if (rc) {
			dev_err(&client->dev, "Failure reading common led configuration, rc = %d\n", rc);
			goto fail_id_check;
		}
		...
		if (strncmp(led_label, "flash", sizeof("flash")) == 0) {
			led->torch_mode = 0;
			if (rc < 0) {
				dev_err(&client->dev, "Unable to read flash config data\n");
				goto fail_id_check;
			}
		} else if (strncmp(led_label, "torch", sizeof("torch")) == 0) {
			led->torch_mode = 1;
			if (rc < 0) {
				dev_err(&client->dev, "Unable to read torch config data\n");
				goto fail_id_check;
			}
		} else {
			dev_err(&client->dev, "No LED matching label\n");
			rc = -EINVAL;
			goto fail_id_check;
		}
		...
		if (rc) {
			dev_err(&client->dev, "Unable to register led %d,rc=%d\n", led->id, rc);
			goto fail_id_check;
		}
	...
	if (err < 0) {
		dev_err(&client->dev, "Register reboot notifier failed(err=%d)\n", err);
		goto platform_data_null;
	}
	...
	if (this_tps61310->enable_FLT_1500mA) {
		dev_dbg(&client->dev, "Flashlight with 1.5A\n");
	...
	if (this_tps61310->reset)
		dev_dbg(&client->dev, "reset pin exist\n");
	else
		dev_dbg(&client->dev, "no reset pin\n");
	if (this_tps61310->power_save) {
		dev_dbg(&client->dev, "power save pin exist\n");
		gpio_set_value_cansleep(this_tps61310->power_save, 0);
	}
	else
		dev_dbg(&client->dev, "o power save pin\n");
	if (this_tps61310->power_save_2) {
		dev_dbg(&client->dev, "power save pin_2 exist\n");
		gpio_set_value_cansleep(this_tps61310->power_save_2, 0);
	}
	else
		dev_dbg(&client->dev, "no power save pin_2\n");
