	...
	loff_t *ppos)
{
	...
	pr_debug("%s: cnt=%d, type=0x%x\n", __func__, cnt, type);

	
	for (i = 0; i < cnt; i++) {
		if (i >= DCS_MAX_CNT) {
			pr_debug("%s: DCS command count over DCS_MAX_CNT, Skip these commands.\n", __func__);
			break;
		}
		tmp = debug_buf + (3 * (i + 1));
		sscanf(tmp, "%x", &value);
...
int mdss_mdp_parse_dt_dspp_pcc_setting(struct platform_device *pdev)
{
	...
	if (dspp_pcc_mode_cnt == 0) {
		...
		pr_debug("%s: dspp_pcc_setting found. count=%d\n", __func__, dspp_pcc_mode_cnt);
	}
...
int htc_backlight_transfer_bl_brightness(int val, struct mdss_panel_info *panel_info, bool brightness_to_bl)
{
	...
	pr_debug("%s: mode=%d, %d transfer to %d\n",
...
int htc_backlight_bl_to_nits(int val, struct mdss_panel_info *panel_info)
{
	...
	pr_debug("%s: bl=%d, nits=%d", __func__, val, nits);
	return nits;
}
...
int htc_backlight_nits_to_bl(int val, struct mdss_panel_info *panel_info)
{
	...
	if (print_log || !timeout) {
		if (suppress_count) {
			pr_debug("%s: nits=%d, bl=%d, suppress %d logs, last value=(%d)",
					__func__, val, code, suppress_count, suppress_nits);
			suppress_count = 0;
		} else {
			pr_debug("%s: nits=%d, bl=%d", __func__, val, code);
		}
