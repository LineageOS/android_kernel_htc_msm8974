...
void mhl_tx_vbus_control(enum vbus_power_state power_state)
{
	...
	if(!reg_boost_5v) {
		...
		if (IS_ERR_OR_NULL(reg_boost_5v)) {
			pr_err("%s: get reg_boost_5v fail, ret:%ld\n", __func__,
...
static void update_mhl_status(bool isMHL, enum usb_connect_type statMHL)
{
	...
	pr_info("MHL is %sConnected, Trigger remove event ++++\n",
			isMHL ? "": "NOT ");
...
static int mhl_get_power_dt_data(struct device *dev)
{
	...
	if (IS_ERR(drv_info.avcc_33_vreg)) {
		pr_err("%s: could not get avcc_33 reg, rc=%ld\n",
	...
	rc = regulator_set_voltage(drv_info.avcc_33_vreg, 3300000,
			3300000);
	if (rc) {
		pr_err("%s: set voltage failed on avcc_33 vreg, rc=%d\n",
			__func__, rc);
		return rc;
	}
	...
	if (IS_ERR(drv_info.avcc_12_vreg)) {
		pr_err("%s: could not get avcc_12 reg, rc=%ld\n",
	...
	rc = regulator_set_voltage(drv_info.avcc_12_vreg, 1200000,
			1200000);
	if (rc) {
		pr_err("%s: set voltage failed on avcc_12 vreg, rc=%d\n",
			__func__, rc);
		return rc;
	}
...
static int mhl_power_on(void)
{
	...
	if (!IS_ERR(drv_info.iovcc_18_vreg)) {
		...
		if (rc) {
			pr_err("%s: Failed to enable iovcc_18_vreg regulator.\n",
				__func__);
			return rc;
		}
	}
	...
	if (rc) {
		pr_err("%s: Failed to enable avcc_33_vreg regulator.\n",
			__func__);
		return rc;
	}
	...
	if (rc) {
		pr_err("%s: Failed to enable avcc_12_vreg regulator.\n",
			__func__);
		return rc;
	}
...
static int mhl_get_gpio_dt_data(struct device *dev)
{
	...
	case DEVICE_ID_8558:
		...
		if (drv_info.fw_wake_pin < 0) {
			pr_debug("%s: Can't get mhl-fw-wake-gpio\n", __func__);
			return -EINVAL;
		}
	...
	if (rc)
		pr_warn("%s: ci2ca_pin not specified\n", __func__);
...
static int mhl_get_swing_value(struct device *dev)
{
	...
	if ((!data) || (len != 3)) {
		pr_warn("%s:%d, Unable to read swing value, use default value\n",
			   __func__, __LINE__);
		for (i = 0; i < len; i++)
					      ...
					      const struct i2c_device_id *id)
{
	...
	if(ret < 0) {
		pr_err("mhl_tx_init failed, error code %d\n", ret);
		return ret;
	}
