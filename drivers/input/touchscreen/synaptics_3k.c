	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "No Synaptics chip\n");
	...
	for (i = 0; i < 10; i++) {
		...
		if (data & 0x44) {
			msleep(20);
#ifdef SYN_FLASH_PROGRAMMING_LOG
			dev_dbg(&client->dev, "F01_data: %x touch controller stay in bootloader mode!\n", data);
#endif
		} else if (data & 0x40) {
			dev_dbg(&client->dev, "F01_data: %x touch controller stay in bootloader mode!\n", data);
			ret = -EIO;
	...
	if (ts->blmode) {
		...
		if (pdata) {
			...
			if (ret < 0) {
				...
				dev_dbg(&client->dev, "syn_config_update success\n");
			else
				dev_dbg(&client->dev, "Warning: syn_config_update: the same "
	...
	if (board_mfg_mode() == MFG_MODE_OFFMODE_CHARGING ||
		...
		dev_dbg(&client->dev, "offmode charging. Set touch chip to sleep mode and skip touch driver probe\n");
		ret = i2c_syn_write_byte_data(ts->client,
	...
	if (client->dev.of_node) {
		...
		if (ret < 0) {
			dev_dbg(&client->dev, "pdata is NULL for DT\n");
			goto err_dt_platform_data_fail;
		}
	} else {
		pdata = client->dev.platform_data;
		if (pdata == NULL) {
			dev_dbg(&client->dev, "pdata is NULL(dev.platform_data)\n");
			ret = -ENOMEM;
...
#endif
		dev_dbg(&client->dev, "pdata->version = %x, pdata->packrat_number = %d, pdata->sensor_id = %x\n",
	...
	if (ret < 0) {
		...
		dev_dbg(&client->dev, "the same config version and CRC\n");
#else
	if (ts->tw_pin_mask) {
		ret = disable_flash_programming(ts, 0);
		if (ret < 0) {
	...
	if (pdata->customer_register[CUS_REG_BASE]) {
		...
		dev_dbg(&client->dev, "Loads customer register\n");
	}
#endif
	...
	if (pdata->display_width && pdata->display_height) {
		dev_dbg(&client->dev, "Load display resolution: %dx%d\n", pdata->display_width, pdata->display_height);
	...
	if (get_tamper_sf() == 0) {
		...
		dev_dbg(&client->dev, "Debug log level=0x%02X\n", ts->debug_log_level);
	}
	...
	if (!gpio_get_value(pdata->gpio_irq)) {
		ret = i2c_syn_read(ts->client, get_address_base(ts, 0x01, DATA_BASE) + 1, &buf, 1);
		dev_dbg(&client->dev, "gpio status = %d, ret = %x\n", gpio_get_value(pdata->gpio_irq), buf);
	}
	if (client->irq) {
		ts->use_irq = 1;
		...
		if (ret == 0) {
			...
			dev_dbg(&client->dev, "interrupt enable: %x\n", ts->intr_bit);
	...
	if (ts->cable_support) {
		...
		dev_dbg(&client->dev, "ts->cable_config: %x\n", ts->cable_config);
	}
#endif
	...
	if (rmi_char_dev_register())
		dev_dbg(&client->dev, "error register char device");
#endif

	dev_dbg(&client->dev, "Start touchscreen %s in %s mode\n",
