...
static int syn_config_update(struct synaptics_ts_data *ts, int attr)
{
	...
	if (crc_checksum == 0xFFFFFFFF) {
		pr_err("%s: crc_checksum Error", __func__);
		return -1;
	}
...
static int synaptics_touch_sysfs_init(void)
{
	...
	if (ret) {
		pr_err("%s: Failed to obtain touchpad IRQ %d. Code: %d.", __func__, ts->gpio_irq, ret);
		return ret;
	}
	if (ts->gpio_reset && !ts->i2c_err_handler_en) {
		ret = gpio_request(ts->gpio_reset, "synaptics_reset");
		if (ret)
			pr_err("%s: Failed to obtain reset pin: %d. Code: %d.", __func__, ts->gpio_reset, ret);
	}
	ret = gpio_export(ts->gpio_irq, true);
	if (ret) {
		pr_err("[TP]%s: Failed to export ATTN gpio!\n", __func__);
		ret = 0;
	} else {
		...
		if (ret) {
			...
			pr_debug("%s: Exported GPIO %d.", __func__, ts->gpio_irq);
		}
	}
#endif
	return 0;
	...
	unsigned long status, void *unused)
{
	...
	pole = (0x2 & status) >> 1;
	pr_debug("%s[%s]", pole? "att_s" : "att_n", pole_value ? "Near" : "Far");
...
static int synaptics_parse_config(struct synaptics_ts_data *ts, struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	while (cfg_table[i].sensor_id > 0 && (cfg_table[i].sensor_id != (SENSOR_ID_CHECKING_EN | ts->tw_vendor))) {
		pr_debug("id:%#x!=%#x, (i++)",cfg_table[i].sensor_id, (SENSOR_ID_CHECKING_EN | ts->tw_vendor));
		i++;
	}
	if (i <= cnt) {
		pr_info("DT-%s cfg idx(%d) in cnt(%d)", __func__, i, cnt);
		...
		pr_info("DT#%d-def_cfg:%d,id:%05x, pr:%d, len:%d,", i,
				...
				struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	if (prop) {
		...
		if (coords_size != 4)
			pr_err("%s:Invalid panel coords size %d", __func__, coords_size);
	}
	...
	if (prop) {
		coords_size = prop->length / sizeof(u32);
		if (coords_size != 4)
			pr_err("%s:Invalid display coords size %d", __func__, coords_size);
	}
	...
	pr_info("DT-%s:display-coords = (%d, %d)", __func__, pdata->display_width,
	...
	if (ts->gpio_i2c >= 0) {
		...
		if (rc)
			pr_err("%s: Failed to obtain i2c_gpio pin: %d. Code: %d.", __func__, ts->gpio_i2c, rc);
	}
	pr_info("DT:gpio_irq=%d, gpio_rst=%d, gpio_i2c=%d", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_i2c);
	...
	if (of_property_read_u32(dt, "synaptics,i2c_err_hlr", &data) == 0) {
		...
		if (pdata->i2c_err_handler_en) {
			...
			if (ts->gpio_reset) {
				...
				if (rc)
					pr_err("%s: Failed to obtain reset pin: %d. Code: %d.", __func__, ts->gpio_reset, rc);
			}
		}
	}
	...
	if (of_property_read_u32(dt, "tw_pin_mask", &data) == 0) {
		ts->tw_pin_mask = data;
		pr_info("DT:tw_pin_mask :%x",ts->tw_pin_mask);
	}

	if (of_property_read_u32(dt, "report_type", &data) == 0) {
		pdata->report_type = data;
		pr_info("DT:report_type=%d", pdata->report_type);
	}

	if (of_property_read_u32(dt, "support_htc_event", &data) == 0) {
		pdata->support_htc_event = data;
		pr_info("DT:support_htc_event=%d", pdata->support_htc_event);
	}

	if (of_property_read_u32(dt, "hall_block_touch_time", &data) == 0) {
		pdata->hall_block_touch_time = data;
		pr_info("DT:hall_block_touch_time=%d", pdata->hall_block_touch_time);
	}
