...
static int i2c_syn_read(struct i2c_client *client, uint16_t addr, uint8_t *data, uint16_t length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		pr_debug("i2c_read retry over %d\n", SYN_I2C_RETRY_TIMES);
		return -EIO;
	}
...
static int i2c_syn_write(struct i2c_client *client, uint16_t addr, uint8_t *data, uint16_t length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		pr_debug("i2c_write retry over %d\n", SYN_I2C_RETRY_TIMES);
		return -EIO;
	}
...
static int i2c_syn_error_handler(struct synaptics_ts_data *ts, uint8_t reset, char *reason, const char *fun_name)
{
	...
	if (reason && fun_name)
		pr_err("I2C Error: %s:%s, reset = %d\n", fun_name, reason, reset);
	else
		pr_debug("%s: reason and fun_name can't be null\n", __func__);

	if (reset) {
		if (ts->power) {
			ret = ts->power(0);
			if (ret < 0)
				pr_err("synaptics i2c error handler power off failed\n");
			msleep(10);
			ret = ts->power(1);
			if (ret < 0)
				pr_err("synaptics i2c error handler power on failed\n");
			ret = synaptics_init_panel(ts);
			if (ret < 0)
				pr_err("synaptics i2c error handler init panel failed\n");
		} else if (ts->gpio_reset) {
			gpio_direction_output(ts->gpio_reset, 0);
			msleep(1);
			gpio_direction_output(ts->gpio_reset, 1);
			pr_debug("%s: synaptics touch chip reseted.\n", __func__);
		}
...
static int i2c_syn_reset_handler(struct synaptics_ts_data *ts, uint8_t reset, char *reason, const char *fun_name)
{
	...
	if (reset) {
		if (ts->power) {
			ret = ts->power(0);
			if (ret < 0)
				pr_err("synaptics i2c reset handler power off failed\n");
			msleep(10);
			ret = ts->power(1);
			if (ret < 0)
				pr_err("synaptics i2c reset handler power on failed\n");
			ret = synaptics_init_panel(ts);
			if (ret < 0)
				pr_err("synaptics i2c reset handler init panel failed\n");
		} else if (ts->gpio_reset) {
			gpio_direction_output(ts->gpio_reset, 0);
			msleep(1);
			gpio_direction_output(ts->gpio_reset, 1);
...
static int wait_flash_interrupt(struct synaptics_ts_data *ts, int attr, int fw)
{
	...
	if (i == 5000 && syn_panel_version == 0) {
		...
		pr_debug("%s: interrupt over time!\n", __func__);
	...
	if (!(data & 0x80)) {
		pr_debug("%s: Not in program enable mode, F34_data = %x\n", __func__, data);
		ret = i2c_syn_read(ts->client,
			get_address_base(ts, 0x01, DATA_BASE), &data, 1);
		pr_debug("%s: Not in program enable mode, 01_data = %x\n", __func__, data);
		return SYN_PROCESS_ERR;
	} else if (data != 0x80) {
		pr_debug("%s: Not in program enable mode, data = %x\n", __func__, data);
	}
	return 0;
}
...
static int enable_flash_programming(struct synaptics_ts_data *ts, int attr)
{
	...
	if (attr) {
		if (!gpio_get_value(attr)) {
			...
			pr_debug("%s: clear gpio_irq before enter bootloader mode, data = %x\n", __func__, data[0]);
		}
	}
...
static int crc_comparison(struct synaptics_ts_data *ts, uint32_t config_crc, int attr)
{
	...
	pr_debug("%s: config_crc = %X, flash_crc = %X\n", __func__, config_crc, flash_crc);
...
static int disable_flash_programming(struct synaptics_ts_data *ts, int status)
{
	...
	if (i == 25) {
		pr_err("%s: Disable flash programming fail! F01_data: %X\n", __func__, data);
		return SYN_PROCESS_ERR;
	} else {
		pr_debug("%s: Disable flash programming success! F01_data: %X\n", __func__, data);
		return status;
	}
}
...
static int syn_config_update(struct synaptics_ts_data *ts, int attr)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (ret == 0)
			...
			pr_debug("%s: Disable flash programming fail %d\n", __func__, retry + 1);
	}

	if (retry == 3) {
		pr_err("%s: Program config fail 3 times\n", __func__);
		return ret;
	}
	return 0;
}
...
static int syn_get_tw_vendor(struct synaptics_ts_data *ts, int attr)
{
	...
	ret = enable_flash_programming(ts, attr);
	if (ret < 0) {
		pr_err("%s: Enable flash programming fail!\n", __func__);
		return disable_flash_programming(ts, -1);
	}

	ret = syn_pdt_scan(ts, SYN_BL_PAGE);
	if (ret < 0) {
		pr_err("%s: pdt scan failed\n", __func__);
		return disable_flash_programming(ts, ret);
	}

	memcpy(&data, &ts->tw_pin_mask, sizeof(ts->tw_pin_mask));
	pr_debug("tw mask = %X %X , %X\n", data[0], data[1], ts->tw_pin_mask);
	data[2] = data[0];
	data[3] = data[1];
...
static int synaptics_input_register(struct synaptics_ts_data *ts)
{
	...
	pr_debug("input_set_abs_params: mix_x %d, max_x %d, min_y %d, max_y %d\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "synaptics-%d_%c.%c",
			ts->package_id, fw_version[1], fw_version[0]);
	if (ts->tw_pin_mask != 0)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		if (buf[0] == 'w') {
			...
			pr_debug("read addr: 0x%X, length: %u\n",
						syn_reg_addr, syn_reg_leng_data);
		}
	} else
		pr_debug("Command Format Error!\n");
		...
		struct device_attribute *attr, char *buf)
{
	...
	wait_event_interruptible_timeout(syn_data_ready_wq, atomic_read(&ts->data_ready), 50);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_debug("Touch: unlock change to %d\n", unlock);
		...
		if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
			...
			if (ts->psensor_detection) {
				if (ts->psensor_resume_enable == 1) {
					pr_debug("%s: Disable P-sensor by Touch\n", __func__);
			...
			if (ts->saturation_bef_unlock) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:4", __func__);
				pr_debug("%s: unlock confirmed. set saturation: %x\n", __func__, ts->saturation_aft_unlock);
			}
			...
			if (ts->PixelTouchThreshold_bef_unlock) {
				...
				pr_debug("%s: set F54_ANALOG_CTRL03 Pixel Touch Threshold: %x\n", __func__, ts->PixelTouchThreshold_aft_unlock);
			}
			...
			if (ts->multitouch_calibration) {
				if (ts->package_id < 3400) {
					...
					pr_debug("%s: Touch Calibration Confirmed, rezero\n", __func__);
				}
			}
#endif
			if (ts->large_obj_check) {
				...
				pr_debug("%s: unlock confirmed. set large obj suppression: %x\n", __func__, ts->default_large_obj);
			}
			...
			if (ts->segmentation_bef_unlock) {
				...
				pr_debug("%s: unlock confirmed. set segmentation aggressiveness: %x\n", __func__, ts->segmentation_aft_unlock);
			}
			...
			if (ts->threshold_bef_unlock) {
				...
				pr_debug("%s: unlock confirmed. set Z Touch threshold: %x\n", __func__, ts->threshold_aft_unlock);
			}
		}
	}
		...
		for (j = 0; j < ts->num_function; j++) {
			if (((ts->address_table[j].control_base >> 8) & 0xFF) == i) {
				...
				if (temp_func_cmd != ts->num_function) {
					...
					pr_debug("page%d has command function, function: %X\n"
						, i, ts->address_table[temp_func_cmd].function_type);
				} else {
					size = ts->address_table[temp_func_query].query_base -
						ts->address_table[j].control_base;
					pr_debug("page%d has no command function, use query function, function: %X\n"
	...
	for (i = 0; i < 20; i++) {
		if (buf[i] == ',' || buf[i] == '\n') {
			...
			if (i - j <= 5)
				...
				pr_debug("buffer size is over 5 char\n");
				return count;
			}
			j = i + 1;
			if (k < 4) {
...
static int register_sr_touch_device(void)
{
	...
	if (ts->sr_input_dev == NULL) {
		pr_err("%s: Failed to allocate SR input device\n", __func__);
						...
						const char *buf, size_t count)
{
	...
	if (buf[0]) {
		if (ts->sr_input_dev)
			pr_debug("%s: SR device already exist!\n", __func__);
		else
			pr_debug("%s: SR touch device enable result:%X\n", __func__, register_sr_touch_device());
	}
	return count;
}
...
static int synaptics_touch_sysfs_init(void)
{
	...
	if (ret) {
		pr_err("%s: Failed to obtain touchpad IRQ %d. Code: %d.\n", __func__, ts->gpio_irq, ret);
		return ret;
	}
	if (ts->gpio_reset && !ts->i2c_err_handler_en) {
		ret = gpio_request(ts->gpio_reset, "synaptics_reset");
		if (ret)
			pr_err("%s: Failed to obtain reset pin: %d. Code: %d.\n", __func__, ts->gpio_reset, ret);
	}
	ret = gpio_export(ts->gpio_irq, true);
	if (ret) {
		pr_err("%s: Failed to export ATTN gpio!\n", __func__);
		ret = 0;
	} else {
		ret = gpio_export_link(&(ts->input_dev->dev), "attn",
			ts->gpio_irq);
		if (ret) {
			pr_err("%s: Failed to symlink ATTN gpio!\n", __func__);
			ret = 0;
		} else {
			pr_debug("%s: Exported GPIO %d.\n", __func__, ts->gpio_irq);
		}
	}
#endif
	return 0;
...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	if (ts->pre_finger_data[0][0] < 2) {
		if (ts->large_obj_check) {
			if (ts->package_id < 3400) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
				pr_debug("%s: set large obj suppression register to: %x\n", __func__, ts->default_large_obj & 0x7F);
			}
		}
		...
		if (ts->segmentation_bef_unlock) {
			if (ts->package_id < 3400) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:3", __func__);
				pr_debug("%s: set segmentation aggressiveness to: %x\n", __func__, ts->segmentation_bef_unlock);
			}
		}
		...
		if (ts->threshold_bef_unlock) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:4", __func__);
			pr_debug("%s: set Z Touch threshold to: %x\n", __func__, ts->threshold_bef_unlock);
		}
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
		if (ts->pre_finger_data[0][0] >= 2 || ts->mfg_flag == 1) {
			...
			pr_debug("%s: Touch init: set fast relaxation to 0x0\n", __func__);
		}
	}
#endif
...
static void synaptics_ts_hover_func(struct synaptics_ts_data *ts, uint8_t *buf, uint8_t *noise_index)
{
	...
	if (!finger_state) {
		...
		z = 255 - buf[5];
		if (ts->debug_log_level & BIT(1))
			pr_debug("Hover=> X:%d, Y:%d, Z:%d\n", x, y, z);
		if (ts->debug_log_level & BIT(17))
			pr_debug("Hover=> X:%d, Y:%d, Z:%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
				x, y, z, temp_im, temp_cidim, noise_index[9], noise_index[4]);
		if ((ts->hover_mode ==0) && (ts->debug_log_level & BIT(3))) {
			if(ts->width_factor && ts->height_factor){
				pr_debug("Screen: Hover:Down, X=%d, Y=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
					(x*ts->width_factor)>>SHIFT_BITS, (y*ts->height_factor)>>SHIFT_BITS, z,
					temp_im, temp_cidim, noise_index[9], noise_index[4]);
			} else {
				pr_debug("Raw: Hover:Down, X=%d, Y=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
					x, y, z, temp_im, temp_cidim, noise_index[9], noise_index[4]);
			}
		}
		ts->hover_mode = 1;
...
static void synaptics_ts_finger_func(struct synaptics_ts_data *ts)
{
	...
	if ((noise_index[9] != ts->prev_Freq) || (noise_index[4] != ts->prev_NS)) {
		pr_debug("Finger Pressed:%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
	...
	if (ret < 0) {
		...
		for (i = 0; i < ts->finger_support; i++) {
			...
			if (finger_state & 0x03) {
				...
				if (finger_state == 0x02)
					pr_debug("Finger state[%d] = 0x02\n", i);
				else if (finger_state == 0x03)
					pr_debug("Finger state[%d] = 0x03\n", i);
			}
		...
		if (ts->debug_log_level & BIT(3)) {
			for (i = 0; i < ts->finger_support; i++) {
				if (finger_release_changed & BIT(i) ) {
					...
					if (ts->width_factor && ts->height_factor) {
						pr_debug("Screen:F[%02d]:Up, X=%d, Y=%d, W=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
							i+1, (x_pos[i]*ts->width_factor)>>SHIFT_BITS,
							(y_pos[i]*ts->height_factor)>>SHIFT_BITS,
							finger_data[i][2], finger_data[i][3],
							temp_im, temp_cidim, noise_index[9], noise_index[4]);
					} else {
						pr_debug("Raw:F[%02d]:Up, X=%d, Y=%d, W=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
							i+1, x_pos[i], y_pos[i],
							finger_data[i][2], finger_data[i][3],
							temp_im, temp_cidim, noise_index[9], noise_index[4]);
					}
		...
		if (ts->pre_finger_data[0][0] < 2 || finger_pressed) {
				...
				if ((finger_pressed | finger_release_changed) & BIT(i)) {
					...
					if (ts->htc_event == SYN_AND_REPORT_TYPE_B && ts->reduce_report_level[0]) {
						if (ts->tap_suppression & BIT(i) && finger_pressed & BIT(i)) {
							...
							if (dx > ts->reduce_report_level[TAP_DX_OUTER] || dy > ts->reduce_report_level[TAP_DY_OUTER]) {
								...
								finger_pressed &= ~BIT(i);
								if (ts->debug_log_level & BIT(1))
									pr_debug("Filtered Finger %d=> X:%d, Y:%d w:%d, z:%d\n",
					...
					if (ts->hall_block_touch_event == 0) {
						...
						if ((finger_press_changed & BIT(i)) && ts->debug_log_level & BIT(3)) {
							if(ts->width_factor && ts->height_factor){
								pr_debug("Screen:F[%02d]:Down, X=%d, Y=%d, W=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
									i+1, (finger_data[i][0]*ts->width_factor)>>SHIFT_BITS,
									(finger_data[i][1]*ts->height_factor)>>SHIFT_BITS,
									finger_data[i][2], finger_data[i][3],
									temp_im, temp_cidim, noise_index[9], noise_index[4]);
							} else {
								pr_debug("Raw:F[%02d]:Down, X=%d, Y=%d, W=%d, Z=%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
									i+1, finger_data[i][0], finger_data[i][1],
									finger_data[i][2], finger_data[i][3],
									temp_im, temp_cidim, noise_index[9], noise_index[4]);
							}
						...
						if (ts->debug_log_level & BIT(1))
							pr_debug("Finger %d=> X:%d, Y:%d W:%d, Z:%d\n",
								i + 1, finger_data[i][0], finger_data[i][1],
								finger_data[i][2], finger_data[i][3]);
						if (ts->debug_log_level & BIT(17))
							pr_debug("Finger %d=> X:%d, Y:%d W:%d, Z:%d, IM:%d, CIDIM:%d, Freq:%d, NS:%d\n",
			...
			if (ts->debug_log_level & BIT(16))
				pr_debug("ts->grip_suppression: %x, ts->ambiguous_state: %x\n",
...
static void synaptics_ts_button_func(struct synaptics_ts_data *ts)
{
	...
	if (data) {
		...
		if (ts->button) {
			...
			if (idx == 100) {
				vk_press = 0;
				pr_err("vk_data:%#x, idx=%d", data, idx);
				return;
			}
		...
		data == 0x01 ? pr_info("back key pressed, vk=%x\n", data) :
		data == 0x02 ? pr_info("home key pressed, vk=%x\n", data) :
		data == 0x04 ? pr_info("app key pressed, vk=%x\n", data) :
		pr_info("vk=%#x\n", data);
...
static void synaptics_ts_status_func(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		...
		data &= 0x0F;
		pr_debug("Device Status = %x\n", data);
		if (data == 1) {
			mutex_lock(&syn_mutex);
			ts->page_select = 0;
			mutex_unlock(&syn_mutex);
			pr_debug("Page Select: %s: %d\n", __func__, ts->page_select);
			ret = synaptics_init_panel(ts);
			if (ret < 0)
				pr_debug("%s: synaptics_init_panel fail\n", __func__);
	...
	if (ret < 0) {
		...
		if (buf & get_address_base(ts, 0x1A, INTR_SOURCE)) {
			if (!ts->finger_count)
				...
				pr_debug("Ignore VK interrupt due to 2d points did not leave\n");
		}
		if (buf & get_address_base(ts, 0x01, INTR_SOURCE))
			synaptics_ts_status_func(ts);
		if (buf & get_address_base(ts, 0x54, INTR_SOURCE))
...
static void cable_tp_status_handler_func(int connect_status)
{
	...
	pr_debug("Touch: cable change to %d\n", connect_status);
...
static int syn_pdt_scan(struct synaptics_ts_data *ts, int num_page)
{
	...
	if (ts->address_table == NULL) {
		...
		pr_debug("syn_pdt_scan: memory allocate success. ptr: %p\n", ts->address_table);
	}

	pr_debug("synaptics: %d function supported\n", ts->num_function);
	for (i = 0; i < num_page; i++) {
...
static int syn_get_information(struct synaptics_ts_data *ts)
{
	...
	if (ts->package_id < 3400) {
		...
		if ((data[0] & 0x07) == 5)
			...
			pr_debug("%s: number of fingers not define: %x\n", __func__, data[0] & 0x07);
			return SYN_PROCESS_ERR;
		}
	} else {
	...
	if (get_address_base(ts, 0x54, FUNCTION)) {
		...
		if (buf == NULL) {
			pr_debug("%s: memory allocate fail\n", __func__);
			return -ENOMEM;
		}
		if (ts->packrat_number < SYNAPTICS_FW_3_2_PACKRAT)
	...
	pr_debug("%s: finger_support:%d, max_x:%d, max_y:%d, X:%d, Y:%d, ts->relaxation:%d", __func__,
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
		if (ts->large_obj_check) {
			if (ts->package_id < 3400) {
				...
				if (ret < 0)
						return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:6", __func__);
				pr_debug("%s: ts->default_large_obj: %x\n", __func__, ts->default_large_obj);
			}
		}
		...
		if (ts->segmentation_bef_unlock) {
			if (ts->package_id < 3400) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:7", __func__);
				pr_debug("%s: ts->segmentation_aft_unlock: %x\n", __func__, ts->segmentation_aft_unlock);
			}
		}
		...
		if (ts->threshold_bef_unlock) {
			if (ts->package_id < 3400) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:8", __func__);
				pr_debug("%s: ts->z_threshold_aft_unlock: %x\n", __func__, ts->threshold_aft_unlock);
			}
		}
		...
		if (ts->saturation_bef_unlock) {
			...
			pr_debug("%s: ts->saturation_aft_unlock: %x\n", __func__, ts->saturation_aft_unlock);
		}
		...
		if (ts->PixelTouchThreshold_bef_unlock) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:9", __func__);
			pr_debug("%s: ts->PixelTouchThreshold_aft_unlock: %x\n", __func__, ts->PixelTouchThreshold_aft_unlock);
		}
	}
...
static int synaptics_parse_config(struct synaptics_ts_data *ts, struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	if (node == NULL) {
		pr_err("%s, can't find device_node\n", __func__);
		return -ENODEV;
	}
	...
	while ((pp = of_get_next_child(node, pp))) {
		...
		if (!prop) {
			pr_debug("%s: Looking up %s property in node %s failed\n",
					__func__, "config", pp->full_name);
			kfree(cfg_table);
			return -ENODEV;
		} else if (!len) {
			pr_debug("%s: Invalid length of configuration data\n", __func__);
			kfree(cfg_table);
			return -EINVAL;
		}
	...
	if (ts->blmode == true) {
		...
		pr_info("Stay BOOTLOADER mode\n");
		pdata->gpio_irq = of_get_named_gpio(dt, "synaptics,irq-gpio", 0);
		if (!gpio_is_valid(pdata->gpio_irq))
			pr_err("DT:gpio_irq value is not valid\n");
		while (cfg_table[i].default_cfg != 1) {
			if (cfg_table[i].default_cfg == 0)
				pr_err("NOT enable recovery method\n");
			i++;
		}
		memcpy(ts->config, cfg_table[i].config, sizeof(cfg_table[i].config));
		ret = syn_config_update(ts, pdata->gpio_irq);
		if (ret < 0) {
			pr_err("syn_config_update fail\n");
			kfree(cfg_table);
			return -EINVAL;
		} else if (ret == 0) {
			pr_info("BL reocver_config_update success\n");
			ts->blmode = false;
			if (syn_get_version(ts) < 0) {
				pr_err("BLmode re-read pr number err\n");
				kfree(cfg_table);
				return -EINVAL;
			}
		} else
			pr_info("%s:the same config version and CRC but touch controller\
	...
	if (board_build_flag() != BUILD_MODE_MFG) {
		...
		pr_info("not mfg config\n");
	}

	while (cfg_table[i].sensor_id > 0 && (cfg_table[i].sensor_id != (SENSOR_ID_CHECKING_EN | ts->tw_vendor))) {
		pr_debug("id:%#x!=%#x, (i++)\n",cfg_table[i].sensor_id, (SENSOR_ID_CHECKING_EN | ts->tw_vendor));
		i++;
	}
	if (i <= cnt) {
		pr_info("DT-%s cfg idx(%d) in cnt(%d)\n", __func__, i, cnt);
		...
		pr_info("DT#%d-def_cfg:%d,id:%05x, pr:%d, len:%d, cfg=[%02x,%02x,%02x,%02x] pl=[x_m%02d y_M%04d]\n", i,
			cfg_table[i].default_cfg, cfg_table[i].sensor_id, cfg_table[i].pr_number, cfg_table[i].length,
			cfg_table[i].config[0], cfg_table[i].config[1], cfg_table[i].config[2], cfg_table[i].config[3],
				...
				struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	if (of_property_read_u32_array(dt, "synaptics,panel-coords", coords, coords_size) == 0) {
		...
		pr_info("DT-%s:panel-coords = %d, %d, %d, %d\n", __func__, pdata->abs_x_min,
	...
	pr_info("DT-%s:display-coords = (%d, %d)\n", __func__, pdata->display_width,
	...
	if (!gpio_is_valid(pdata->gpio_reset)) {
		pr_err("DT:gpio_rst value is not valid\n");
	}

	pdata->gpio_i2c = of_get_named_gpio(dt, "synaptics,i2c-gpio", 0);
	if (!gpio_is_valid(pdata->gpio_i2c)) {
		pr_err("DT:gpio_i2c value is not valid\n");
	}
	ts->gpio_i2c = pdata->gpio_i2c;
	if (ts->gpio_i2c >= 0) {
		rc = gpio_request(ts->gpio_i2c, "synaptics_i2c_gpio");
		if (rc)
			pr_err("%s: Failed to obtain i2c_gpio pin: %d. Code: %d.\n", __func__, ts->gpio_i2c, rc);
	}
	pr_info("DT:gpio_irq=%d, gpio_rst=%d, gpio_i2c=%d\n", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_i2c);
	...
	if (rc < 0) {
		pr_err("DT:cfg table parser FAIL. ret=%d\n", rc);
		return rc;
	} else if (rc == 0)
		pr_info("DT parser Done\n");
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_info(&client->dev, "No Synaptics chip\n");
		goto err_detect_failed;
	}

	if (syn_pdt_scan(ts, SYN_BL_PAGE) < 0) {
		dev_err(&client->dev, "PDT scan fail\n");
		ret = -EIO;
	...
	for (i = 0; i < 10; i++) {
		...
		if (data & 0x44) {
			...
			dev_info(&client->dev, "F01_data: %x touch controller stay in bootloader mode!\n", data);
			ret = -EIO;
			goto err_detect_failed;
		} else
			break;
	...
	if (ts->blmode) {
		...
		uint8_t data1[3] = {0};
#endif
		dev_err(&client->dev, "touch controller doesn't enter UI mode! F01_data: %x\n", data);
		if (syn_pdt_scan(ts, SYN_BL_PAGE) < 0) {
			dev_err(&client->dev, "PDT scan fail\n");
			ret = -EIO;
			goto err_init_failed;
		...
		if (pdata) {
			...
			while (pdata->default_config != 1) {
				if (pdata->default_config == 0) {
					dev_err(&client->dev, "touch controller stays in bootloader mode "
			...
			ret = syn_config_update(ts, pdata->gpio_irq);
			if (ret < 0) {
				dev_err(&client->dev, "syn_config_update fail\n");
				goto err_init_failed;
			} else if (ret == 0)
				dev_info(&client->dev, "syn_config_update success\n");
			else
				dev_info(&client->dev, "Warning: syn_config_update: the same "
						"config version and CRC but touch controller "
						"always stay in bootloader mode\n");
			pdata = pdata - num;
		}
#else
		...
		if (ret < 0) {
			dev_err(&client->dev, "read packrat_number fail\n");
			goto err_init_failed;
		}
		...
		if (ret < 0) {
			dev_err(&client->dev, "config update fail\n. ret=%d\n", ret);
			goto err_init_failed;
		} else if (ret == 0)
			dev_dbg(&client->dev, "config update done\n");
		kfree(pdata);
#endif
	...
	if (board_mfg_mode() == MFG_MODE_OFFMODE_CHARGING ||
		...
		dev_info(&client->dev, "offmode charging. Set touch chip to sleep mode and skip touch driver probe\n");
	...
	if (syn_get_version(ts) < 0) {
		dev_err(&client->dev, "syn_get_version fail\n");
		ret = -EIO;
		goto err_init_failed;
	}
	...
	if (client->dev.of_node) {
		...
		if (ret < 0) {
			dev_err(&client->dev, "pdata is NULL for DT\n");
			goto err_dt_platform_data_fail;
		}
	} else {
		pdata = client->dev.platform_data;
		if (pdata == NULL) {
			dev_err(&client->dev, "pdata is NULL(dev.platform_data)\n");
			ret = -ENOMEM;
	...
	if (pdata) {
		...
		if (pdata->tw_pin_mask) {
			...
			if (ret < 0) {
				dev_err(&client->dev, "syn_get_tw_vendor fail\n");
				goto err_init_failed;
			}
		}
...
#endif
		dev_info(&client->dev, "pdata->version = %x, pdata->packrat_number = %d, pdata->sensor_id = %x\n",
				pdata->version, pdata->packrat_number, pdata->sensor_id);

		if (!pdata->packrat_number) {
			dev_err(&client->dev, "get null platform data\n");
			ret = -ENODEV;
			goto err_init_failed;
		}
		...
		if (pdata->virtual_key) {
			...
			dev_dbg(&client->dev, "vk cnt:%d:%s\n%s\n", i, ts->vkey_buf, ts->vkey_buf+pos);
			syn_init_vkeys();
		}
	}
	dev_dbg(&client->dev, "config =[%02x,%02x,%02x,%02x] \n",
	...
	if (ret < 0) {
		...
		dev_info(&client->dev, "the same config version and CRC\n");
#else
	if (ts->tw_pin_mask) {
		ret = disable_flash_programming(ts, 0);
		if (ret < 0) {
			dev_err(&client->dev, "disable_flash_programming fail\n");
			goto err_init_failed;
		}
	}
#endif
	...
	if (pdata->customer_register[CUS_REG_BASE]) {
		...
		dev_info(&client->dev, "Loads customer register\n");
	}
#endif
	if (syn_get_information(ts) < 0) {
		dev_err(&client->dev, "syn_get_information fail\n");
		ret = -EIO;
	...
	if (pdata->display_width && pdata->display_height) {
		dev_info(&client->dev, "Load display resolution: %dx%d\n", pdata->display_width, pdata->display_height);
	...
	if (get_tamper_sf() == 0) {
		...
		dev_info(&client->dev, "Debug log level=0x%02X\n", ts->debug_log_level);
	}
	...
	if (get_address_base(ts, 0x19, FUNCTION)) {
		...
		for (i = 0; i < ts->key_number; i++) {
			ts->key_postion_x[i] =
				...
				+ ts->layout[0];
			dev_dbg(&client->dev, "ts->key_postion_x[%d]: %d\n",
				i, ts->key_postion_x[i]);
		}
		ts->key_postion_y = ts->layout[2] +
			(21 * (ts->layout[3] - ts->layout[2]) / 20);
		dev_dbg(&client->dev, "ts->key_postion_y: %d\n", ts->key_postion_y);
	}

	ret = synaptics_init_panel(ts);
	if (ret < 0) {
		dev_err(&client->dev, "synaptics_init_panel fail\n");
	...
	if (!gpio_get_value(pdata->gpio_irq)) {
		ret = i2c_syn_read(ts->client, get_address_base(ts, 0x01, DATA_BASE) + 1, &buf, 1);
		dev_info(&client->dev, "gpio status = %d, ret = %x\n", gpio_get_value(pdata->gpio_irq), buf);
	}
	if (client->irq) {
		ts->use_irq = 1;
		...
		if (ret == 0) {
			...
			dev_info(&client->dev, "interrupt enable: %x\n", ts->intr_bit);
	...
	if (!ts->syn_att_wq) {
		dev_err(&client->dev, "allocate syn_att_wq failed\n");
		ret = -ENOMEM;
		goto err_get_intr_bit_failed;
	}
	...
	if (rmi_char_dev_register())
		dev_err(&client->dev, "error register char device\n");
#endif
...
static int synaptics_ts_suspend(struct device *dev)
{
	...
	if (!probe_success) {
		pr_debug("%s: Driver not probe\n", __func__);
		return 0;
	}

	if (ts->suspended) {
		pr_debug("%s: Already suspended. Skipped. FW:%d;%d;%x;%x\n", __func__,
				ts->package_id, ts->packrat_number, syn_panel_version, ts->config_version);
		return 0;
	} else {
		ts->suspended = true;
		pr_debug(" %s: enter. FW:%d;%d;%x;%x\n", __func__,
				ts->package_id, ts->packrat_number, syn_panel_version, ts->config_version);
	}
		...
		if (ts->psensor_status == 0) {
			...
			if (ts->mfg_flag != 1) {
				...
				if (ts->saturation_bef_unlock) {
					...
					if (ret < 0)
						return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "saturation capacitance", __func__);
					pr_debug("touch suspend, saturation capacitance: %x\n", ts->saturation_bef_unlock);
				}
				...
				if ( ts->PixelTouchThreshold_bef_unlock ) {
					if (ts->package_id <= 3400 ) {
						...
						if (ret < 0)
							return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "F54_ANALOG_CTRL03 Pixel Touch Threshold", __func__);
						pr_debug("touch suspend, set F54_ANALOG_CTRL03 Pixel Touch Threshold: %x\n", ts->PixelTouchThreshold_bef_unlock);
					}
				}
				...
				pr_debug("touch suspend, fast relasxation: %x\n", ts->relaxation);
			}
#endif
			...
			if (ts->large_obj_check) {
				if (ts->package_id < 3400) {
					...
					pr_debug("touch suspend, set large obj suppression: %x\n", ts->default_large_obj & 0x7F);
				}
			}
			...
			if (ts->segmentation_bef_unlock) {
				if (ts->package_id < 3400) {
					...
					pr_debug("touch suspend, set segmentation aggressiveness: %x\n", ts->segmentation_bef_unlock);
				}
			}
			...
			if (ts->threshold_bef_unlock) {
				if (ts->package_id < 3400) {
					...
					pr_debug("touch suspend, set Z Touch threshold: %x\n", ts->threshold_bef_unlock);
				}
			}
...
static int synaptics_ts_resume(struct device *dev)
{
	...
	pr_debug("%s: enter. FW:%d;%d;%x;%x\n",
			__func__, ts->package_id, ts->packrat_number,
			syn_panel_version, ts->config_version);
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
		if (ts->psensor_detection) {
			if (ts->psensor_status == 0) {
				...
				pr_debug("%s: Enable P-sensor by Touch\n", __func__);
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		case FB_BLANK_NORMAL:
			if (ts->syn_cover_wq) {
				pr_debug("Suspend, flush cover workqueue\n");
