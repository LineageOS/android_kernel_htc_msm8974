					 ...
					 const char *buf, size_t count)
{
	...
	if (wakeup_bitmask && (!set_wakeup)) {
		...
		set_wakeup = 1;
		pr_debug("%s: change to wake up function(%d, %d)\n",
		...
		set_wakeup = 0;
		pr_debug("%s: change to non-wake up function(%d, %d)\n",
				__func__, vol_up_irq, vol_down_irq);
	}
...
static void power_key_state_disable_work_func(struct work_struct *dummy)
{
   ...
   pr_debug("Power key pressed outdated\n");
}
...
static void handle_power_key_state(unsigned int code, int value)
{
	...
	if (code == KEY_POWER && value == 1) {
		pr_debug("Try to schedule power key pressed due\n");
		ret = schedule_delayed_work(&power_key_state_disable_work, PWRKEY_PRESS_DUE);
		if (!ret) {
			pr_debug("Schedule power key pressed due failed, seems already have one, try to cancel...\n");
			...
			if (!ret) {
				...
				if (schedule_delayed_work(&power_key_state_disable_work, PWRKEY_PRESS_DUE)) {
					pr_debug("Re-schedule power key pressed due SCCUESS.\n");
					pr_debug("Start count for power key pressed due\n");
					setPowerKeyState(1);
				} else
					pr_debug("Re-schedule power key pressed due FAILED, reason unknown, give up.\n");
			} else {
				pr_debug("Cancel scheduled power key due success, now re-schedule.\n");
				if (schedule_delayed_work(&power_key_state_disable_work, PWRKEY_PRESS_DUE)) {
					pr_debug("Re-schedule power key pressed due SCCUESS.\n");
					pr_debug("Start count for power key pressed due\n");
					setPowerKeyState(1);
...
#if defined(CONFIG_PM8921_BMS) && (CONFIG_HTC_BATT_8974)
...
static void clear_hw_reset(void)
{
	...
	if ((aa->clear_hw_reset)) {
		...
		pr_debug("No reset clear function\n");
}
...
static void power_key_restart_work_func(struct work_struct *dummy)
{
	...
	pr_info("%s: power_key_check_in_pocket = %d\n", __func__, pocket_mode);
	...
	if (gpio_val && pre_power_key_led_status == 1 && !is_rrm1_mode()) {
		pr_info("%s, (PWR+VOL_UP) reset", __func__);
#endif
		set_hw_reason(0);
#if defined(CONFIG_PM8921_BMS) && (CONFIG_HTC_BATT_8974)
...
static void power_key_led_on_work_func(struct work_struct *dummy)
{
	...
	if (power_key_led_requested == 1) {
...
#endif
		pr_debug("Change power key led on\n");
#ifdef CONFIG_QPNP_PWM
#ifdef CONFIG_VK_LED
		...
		pr_debug("Show Blocked State -- long press power key\n");
...
static void power_key_led_off_work_func(struct work_struct *dummy)
{
	if (power_key_led_requested) {
		if (cancel_delayed_work_sync(&power_key_led_on_work)) {
			pr_debug("Cancel power key led work successfully(%x)\n", power_key_led_requested);
		} else
			pr_err("Cancel power key led work unsuccessfully (%x)\n", power_key_led_requested);
...
#endif
		pr_debug("Change power key led off\n");
#ifdef CONFIG_QPNP_PWM
#ifdef CONFIG_VK_LED
...
static void power_key_check_reset_work_func(struct work_struct *dummy)
{
	...
	if ((aa->dt_clear_hw_reset)) {
		if (aa->info.rrm1_mode) {
			pr_debug("Power key check in Lab Test RRM1 mode.\n");
			aa->dt_clear_hw_reset(aa->clr_gpio);
#else
	if ((aa->clear_hw_reset)) {
		if (aa->info.rrm1_mode) {
			...
			if (pocket_mode) {
				pr_debug("power_key_check_in_pocket = %d\n", pocket_mode);
#ifdef CONFIG_OF
				aa->dt_clear_hw_reset(aa->clr_gpio);
#else
				aa->clear_hw_reset();
			...
			pr_debug("Not in normal OS mode, mode=%d\n", board_mfg_mode());
#else
		} else {
			pr_debug("OS Mode=%d\n", board_mfg_mode());
...
static void power_key_clr_check_work_func(struct work_struct *dummy)
{
	...
	if (val) {
		...
		if (cancel_delayed_work_sync(&power_key_check_reset_work))
			pr_debug("Cancel power key check reset work successfully\n");
		else
			pr_err("Cancel power key check reset work unsuccessfully\n");
#ifdef CONFIG_KPDPWR_S2_DVDD_RESET
		clear_kpdpwr_s2_rst_flag = 0;
		pr_debug("%s: Disable kpdpwr s2 reset clear up [%d]\n", __func__, clear_kpdpwr_s2_rst_flag);
	...
	if (code == KEY_POWER || code == KEY_VOLUMEUP) {
		...
		for (i = 0; (i < aa->keymap_size && code != KEY_POWER); i++) {
			if (aa->keymap[i].code == KEY_POWER) {
				...
				if (read_val) {
					...
					pr_debug("[PWR+VUP] start count for power key led on\n");
					schedule_delayed_work(&power_key_led_on_work, PWRKEYLEDON_DELAY);
					power_key_led_requested = 1;
#ifdef CONFIG_KPDPWR_S2_DVDD_RESET
					clear_kpdpwr_s2_rst_flag = 1;
					pr_debug("%s: Enable kpdpwr s2 reset clear up [%d]\n",
								__func__, clear_kpdpwr_s2_rst_flag);
#endif 
					break;
				}
		...
		for (i = aa->keymap_size; (i > 0  && code != KEY_VOLUMEUP); i--) {
			if (aa->keymap[i-1].code == KEY_VOLUMEUP) {
				...
				pr_debug("Idx[%d] GPIO_%d:VOL_UP is %s\n",
						i-1, aa->keymap[i-1].gpio,
						(read_val ? "NOT pressed" : "PRESSED" ));
				if (read_val) {
					goto KEY_PWR;
				} else {
					pr_debug("[VUP+PWR] start count for power key led on\n");
					schedule_delayed_work(&power_key_led_on_work, PWRKEYLEDON_DELAY);
					power_key_led_requested = 1;
					break;
				}
...
KEY_PWR:
	...
	if (code == KEY_POWER) {
		...
		if (value) {
			...
			pr_debug("Start count for power key check reset\n");
...
#endif
				pr_debug("The reset work in already in the queue\n");
		} else {
			pr_debug("Start count for power key clear check\n");

			if (!schedule_delayed_work(&power_key_clr_check_work, PWRKEYCLRCHK_DELAY))
				pr_debug("The clear work in already in the queue\n");
#ifdef CONFIG_KPDPWR_S2_DVDD_RESET
			clear_kpdpwr_s2_rst_flag = 0;
			pr_debug("%s: Disable kpdpwr s2 reset clear up [%d]\n",
						__func__, clear_kpdpwr_s2_rst_flag);
...
static int disable_reset_sysfs_init(void)
{
	...
	if (ret) {
		pr_err("%s: sysfs_create_file disable_reset failed\n", __func__);
		return ret;
	}
...
void keypad_report_keycode(struct gpio_key_state *ks)
{
	...
	if (key_entry == NULL) {
		pr_err("%s, (key_entry == NULL) failed\n", __func__);
		return;
	}
	...
	if (ds->info->flags & GPIOEDF_PRINT_KEYS)
		pr_debug("%s: key %d-%d, %d (%d) changed to %d\n",
				__func__, ds->info->type, key_entry->code,
...
static irqreturn_t gpio_event_input_irq_handler(int irq, void *dev_id)
{
	...
	if (key_entry->code == KEY_POWER && power_key_intr_flag == 0) {
		...
		pr_debug("%s, keycode = %d, first intr", __func__, key_entry->code);
	}
...
static int gpio_event_input_request_irqs(struct gpio_input_state *ds)
{
	...
	for (i = 0; i < ds->info->keymap_size; i++) {
		...
		if (ds->info->keymap[i].code == KEY_VOLUMEUP ||
				...
				ds->info->keymap[i].code == KEY_HP ) {
			pr_info("keycode = %d, gpio = %d, irq = %d",
					ds->info->keymap[i].code, ds->info->keymap[i].gpio, irq);
			if (ds->info->keymap[i].code == KEY_VOLUMEUP)
				vol_up_irq = irq;
			...
			struct gpio_event_info *info, void **data, int func)
{
	...
	if (func == GPIO_EVENT_FUNC_INIT) {
		...
		if (di->info.rrm1_mode && di->dt_clear_hw_reset) {
			pr_debug("First clear reset in Lab Test RRM1 mode.\n");
		...
		*data = ds = kzalloc(sizeof(*ds) + sizeof(ds->key_state[0]) *
		...
		if (sysfs_create_file(keyboard_kobj, &dev_attr_vol_wakeup.attr))
			pr_err("%s: sysfs_create_file return %d\n", __func__, ret);
		...
		if(board_mfg_mode() != MFG_MODE_NORMAL) {
			...
			if (disable_reset_sysfs_init())
				pr_err("%s: disable_reset_sysfs_init fail\n", __func__);
		}
		ds->disable_reset_flag = 0;
