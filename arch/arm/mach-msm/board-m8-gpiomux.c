...
void __init msm_htc_8974_init_gpiomux(void)
{
	...
	msm_gpiomux_install(msm_sensor_configs_non_common,
	...
	if (of_machine_pid() == m8_UHL_PID)
		msm_gpiomux_install(msm_sensor_configs_china_sku,
        	...
        	msm_gpiomux_install(msm_sensor_configs_non_china_sku,
				ARRAY_SIZE(msm_sensor_configs_non_china_sku));
	...
	msm_gpiomux_install(msm_aud_spk_china_configs,
			ARRAY_SIZE(msm_aud_spk_china_configs));

	msm_gpiomux_install(msm_cir_configs,
			ARRAY_SIZE(msm_cir_configs));

	if (of_machine_pid() == 268 || of_board_is_m8wlj())
		msm_gpiomux_install(msm_felica_configs,
				ARRAY_SIZE(msm_felica_configs));
