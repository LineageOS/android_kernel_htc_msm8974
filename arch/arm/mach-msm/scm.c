				...
				size_t scm_buf_length)
{
	...
	if (ret) {
		pr_err("%s(%d, %d, [%p, %d], [%p, %d], [%p, %d])\n",
			__func__, svc_id, cmd_id, cmd_buf, cmd_len,
			resp_buf, resp_len, scm_buf, scm_buf_length);
		print_hex_dump(KERN_ERR, "scm_buf: ",
			DUMP_PREFIX_ADDRESS, 16, 4, scm_buf, scm_buf_length, false);
		return ret;
	}
