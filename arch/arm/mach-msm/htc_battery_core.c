				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set charger_control:%lu", enable);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("set context phone_call=%lu", phone_call);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("set context play music=%lu", play_music);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context network_search=%lu", network_search);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context navigation=%lu", navigation);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context event = %lu", event);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context trigger_flag = %lu", trigger_flag);
					...
					char *buf)
{
	...
	if (!battery_core_info.func.func_get_batt_rt_attr) {
		pr_err("%s: func_get_batt_rt_attr does not exist", __func__);
		return -EINVAL;
	}

	rc = battery_core_info.func.func_get_batt_rt_attr(attr_index, &val);
	if (rc) {
		pr_err("%s: get_batt_rt_attrs[%d] failed", __func__, attr_index);
		return -EINVAL;
	}
					...
					const char *buf, size_t count)
{
	...
	if (time_out > 0) {
		...
		alarm_start_range(&batt_charger_ctrl_alarm, next_alarm, next_alarm);
...
{
	power_supply_changed(&htc_power_supplies[BATTERY_SUPPLY]);
	pr_debug("%s: power_supply_changed: battery", __func__);
}
