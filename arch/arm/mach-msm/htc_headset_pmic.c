...
static int hs_pmic_remote_threshold(uint32_t adc)
{
	...
	pr_debug("Set remote threshold (%u, %u, %u)\n", hi->pdata.hs_controller,
...
static int headset_pmic_parser_dt(struct htc_hs_pmic_data *pmic)
{
	...
	if (ret < 0) {
		...
		pr_debug("DT:adc_channel = %d\n", pmic->pdata->adc_channel);
#endif
	...
	if (ret < 0) {
		...
		pr_debug("DT:adc: %03d %03d %03d %03d %03d\n", pmic->pdata->adc_remote[1],
	...
	for (i = 0; i < ARRAY_SIZE(parser_st); i++) {
		...
		if (!gpio_is_valid(gpio[i])) {
			pr_err("DT %s parser failue, ret=%d\n", parser_st[i], gpio[i]);
			goto parser_failed;
		} else
			pr_debug("DT:%s[%d] read\n", parser_st[i], gpio[i]);
	}
...
static int htc_headset_pmic_probe(struct platform_device *pdev)
{
	...
	if (hi->pdata.hpin_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request PMIC HPIN IRQ (0x%X)\n", ret);
	...
	if (hi->pdata.key_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request PMIC button IRQ (0x%X)\n", ret);
	...
	if (hi->pdata.driver_flag & DRIVER_HS_PMIC_DYNAMIC_THRESHOLD) {
		...
		if (IS_ERR(endpoint_current)) {
			...
			pr_debug("Register threshold RPC client successfully (0x%X)\n", vers);
	}
#else
	hi->pdata.driver_flag &= ~DRIVER_HS_PMIC_RPC_KEY;
