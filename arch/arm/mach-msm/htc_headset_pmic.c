...
static int hs_pmic_remote_threshold(uint32_t adc)
{
	...
	if (ret < 0) {
		pr_err("Failed to send remote threshold RPC");
		return 0;
	} else {
		status = be32_to_cpu(rep.status);
		if (status != HS_PMIC_RPC_ERR_SUCCESS) {
			pr_err("Failed to set remote threshold");
			return 0;
		}
	}

	pr_debug("Set remote threshold (%u, %u, %u)", hi->pdata.hs_controller,
...
static int hs_pmic_remote_adc(int *adc)
{
	...
	if (ret < 0) {
		*adc = -1;
		pr_err("Failed to read remote ADC");
		return 0;
	}

	*adc = (int) be32_to_cpu(rep.adc);
	pr_debug("Remote ADC %d (%#X)", *adc, *adc);
...
static int hs_qpnp_remote_adc(int *adc)
{
	...
	pr_debug("pdata_chann %d", chan);
	err = qpnp_vadc_read(vadc_chip, chan, &result);
	if (err < 0) {
		pr_err("Read ADC fail, ret = %d", err);
		return err;
	}
...
static int hs_pmic_adc_to_keycode(int adc)
{
	...
	if (key_code != HS_MGR_KEY_INVALID)
		pr_debug("Key code %d", key_code);
	else
		pr_debug("Unknown key code %d", key_code);
...
static irqreturn_t detect_irq_handler(int irq, void *data)
{
	...
	pr_debug("HPIN++%d++, value = %d, trigger_type = 0x%x",
			hpin_count_local, detect_pmic_work.insert, hi->hpin_irq_type);
	hs_notify_hpin_irq();
	...
	pr_debug("HPIN--%d--, insert = %d, trigger_type = 0x%x",
			hpin_count_local, detect_pmic_work.insert, hi->hpin_irq_type);
	return IRQ_HANDLED;
}
...
static void irq_init_work_func(struct work_struct *work)
{
	...
	if (hi->pdata.key_gpio) {
		pr_debug("Setup button IRQ type");
		hi->key_irq_type = irq_type;
		set_irq_type(hi->pdata.key_irq, hi->key_irq_type);
		if (set_irq_wake(hi->pdata.key_irq, 0) < 0)
			pr_err("Disable remote key irq wake failed");
	}
}
...
static void hs_pmic_key_int_enable(int enable)
{
	...
	if (enable == 1 && enable_count == 0) {
		...
		if (set_irq_wake(hi->pdata.key_irq, 1) < 0)
			pr_debug("Enable remote key irq wake failed");
		pr_debug("Enable remote key irq and wake");
	} else if (enable == 0 && enable_count == 1) {
		disable_irq_nosync(hi->pdata.key_irq);
		enable_count--;
		if (set_irq_wake(hi->pdata.key_irq, 0) < 0)
			pr_debug("Disable remote key irq wake failed");
		pr_debug("Disable remote key irq and wake");
	}
	pr_debug("enable_count = %d", enable_count);
}
			       ...
			       const char *name, unsigned int wake)
{
	...
	if (!(*irq)) {
		...
		pr_debug("gpio_to_irq ret = %d", ret);
...
err_create_pmic_device_file:
	...
	pr_err("Failed to register pmic attribute file");
	return ret;
}
...
static int headset_pmic_parser_dt(struct htc_hs_pmic_data *pmic)
{
	...
	if (ret < 0) {
		pr_err("DT:%s parser err, ret=%d", parser_fg, ret);
		goto parser_failed;
	} else
		pr_debug("DT:%s=%d", parser_fg, pmic->pdata->driver_flag);

#ifndef HTC_HEADSET_CONFIG_QPNP_ADC
	ret = of_property_read_u32(dt, "hs_pmic,adc_mpp", &pmic->pdata->adc_mpp);
	if (ret < 0) {
		pr_err("DT:adc_mpp parser err");
		goto parser_failed;
	} else
		pr_debug("DT:adc_mpp = %d", pmic->pdata->adc_mpp);

	ret = of_property_read_u32(dt, "hs_pmic,adc_amux", &pmic->pdata->adc_amux);
	if (ret < 0) {
		pr_err("DT:adc_amux parser err");
		goto parser_failed;
	} else
		pr_debug("DT:adc_amuxp = %d", pmic->pdata->adc_amux);
#else
	ret = of_property_read_u32(dt, parser_chann, &pmic->pdata->adc_channel);
	if (ret < 0) {
		pr_err("DT:8974 adc channel parser err");
		goto parser_failed;
	} else
		pr_debug("DT:adc_channel = %d", pmic->pdata->adc_channel);
#endif
	...
	if (ret < 0) {
		...
		pr_debug("DT:adc: %03d %03d %03d %03d %03d", pmic->pdata->adc_remote[1],
	...
	for (i = 0; i < ARRAY_SIZE(parser_st); i++) {
		...
		if (!gpio_is_valid(gpio[i])) {
			pr_err("DT %s parser failue, ret=%d", parser_st[i], gpio[i]);
			goto parser_failed;
		} else
			pr_debug("DT:%s[%d] read", parser_st[i], gpio[i]);
	}
...
static int htc_headset_pmic_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		pr_debug("==DT parser %s==", dtinfo->parser ? "OK": "FAILED");
	} else {
	...
	if (hi->pdata.hpin_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request PMIC HPIN IRQ (0x%X)", ret);
	...
	if (hi->pdata.key_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request PMIC button IRQ (0x%X)", ret);
	...
	if (hi->pdata.driver_flag & DRIVER_HS_PMIC_RPC_KEY) {
		...
		if (IS_ERR(endpoint_adc)) {
			...
			pr_err("Failed to register ADC RPC client");
		} else
			pr_debug("Register ADC RPC client successfully");
	}
	...
	if (hi->pdata.driver_flag & DRIVER_HS_PMIC_DYNAMIC_THRESHOLD) {
		...
		if (IS_ERR(endpoint_current)) {
			...
			pr_err("Failed to register threshold RPC client");
		} else
			pr_debug("Register threshold RPC client successfully (0x%X)", vers);
	}
#else
	hi->pdata.driver_flag &= ~DRIVER_HS_PMIC_RPC_KEY;
...
err_create_detect_work_queue:
	...
	pr_err("Failed to register %s driver", DRIVER_NAME);
