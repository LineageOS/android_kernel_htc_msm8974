...
int htc_charger_event_notify(enum htc_charger_event event)
{
	...
	case HTC_CHARGER_EVENT_SRC_INTERNAL:
		...
		pr_debug("htc_ext_5v_output_now:%d", htc_ext_5v_output_now);
		htc_batt_schedule_batt_info_update();
#if defined(CONFIG_MACH_DUMMY)
...
#endif
{
	...
	pr_debug("%s(lw = %lu, up = %lu)", __func__, lower_threshold, upper_threshold);
	rc = pm8058_batt_alarm_state_set(0, 0);
	if (rc) {
		pr_err("state_set disabled failed, rc=%d", rc);
		goto done;
	}

	rc = pm8058_batt_alarm_threshold_set(lower_threshold, upper_threshold);
	if (rc) {
		pr_err("threshold_set failed, rc=%d!", rc);
		goto done;
	}
	...
	if (rc) {
		pr_err("state_set enabled failed, rc=%d", rc);
		goto done;
	}
#endif
...
static int batt_clear_voltage_alarm(void)
{
	...
	pr_debug("disable voltage alarm");
	if (rc)
		pr_err("state_set disabled failed, rc=%d", rc);
	return rc;
}
					...
					unsigned long status, void *data)
{
	...
	if (battery_vol_alarm_mode == BATT_ALARM_CRITICAL_MODE) {
		...
		pr_debug("NORMAL_MODE batt alarm status = %u",
				htc_batt_timer.batt_alarm_status);
	} else
		pr_warn("batt alarm triggerred in disable mode ");
#else
	htc_batt_timer.batt_alarm_status++;
	pr_debug("batt alarm status %u", htc_batt_timer.batt_alarm_status);
#endif
	return 0;
}
#endif
...
static void cable_status_notifier_func(enum usb_connect_type online)
{
	...
	if (online == latest_chg_src && !first_update) {
		pr_debug("%s: charger type (%u) same return.", __func__, online);
		mutex_unlock(&cable_notifier_lock);
		return;
	}
	first_update = 0;
	...
	case CONNECT_TYPE_NOTIFY:
		...
		break;
	default:
		pr_debug("unsupported connect_type=%d", online);
		htc_charger_event_notify(HTC_CHARGER_EVENT_SRC_NONE);
		break;
	}
#if 0 
	htc_batt_timer.alarm_timer_flag = (unsigned int)htc_batt_info.rep.charging_source;
...
static void set_limit_charge_with_reason(bool enable, int reason)
{
	...
	if (chg_limit_active_mask & reason) {
		...
		if (prev_chg_limit_reason ^ chg_limit_reason) {
			pr_debug("chg_limit_reason:0x%x->0x%d", prev_chg_limit_reason, chg_limit_reason);
			if (!!prev_chg_limit_reason != !!chg_limit_reason &&
...
static int htc_batt_context_event_handler(enum batt_context_event event)
{
	...
	pr_debug("context_state: 0x%x -> 0x%x", prev_context_state, context_state);
...
static int htc_batt_charger_control(enum charger_control_flag control)
{
	...
	pr_debug("%s: user switch charger to mode: %u", __func__, control);
	...
	case DISABLE_LIMIT_CHARGER:
		pr_debug("%s: skip charger_contorl(%d)", __func__, control);
		return ret;
		break;
	default:
		pr_debug("%s: unsupported charger_contorl(%d)", __func__, control);
		ret =  -1;
		break;
	}
...
static int htc_batt_ftm_charger_control(enum ftm_charger_control_flag control)
{
	...
	if (control < FTM_END_CHARGER)
		...
		pr_debug("%s: unsupported ftm_charger_contorl(%d)", __func__, control);
		ret =  -1;
	}
...
static void htc_batt_set_full_level(int percent)
{
	...
	pr_debug("set full_level constraint as %d.", percent);
...
static void htc_batt_set_full_level_dis_batt_chg(int percent)
{
	...
	pr_debug("set full_level_dis_batt_chg constraint as %d.", percent);
...
static int htc_batt_open(struct inode *inode, struct file *filp)
{
	...
	pr_debug("%s: open misc device driver.", __func__);
...
{
	pr_debug("%s: release misc device driver.", __func__);
	spin_lock(&htc_batt_info.batt_lock);
...
static int htc_batt_get_battery_info(struct battery_info_reply *htc_batt_update)
{
	...
	htc_batt_update->batt_discharg_current = htc_batt_info.rep.batt_discharg_current;
#endif
...
static int32_t htc_batt_get_battery_adc(void)
{
	...
	ret = pm8058_htc_config_mpp_and_adc_read(adc.adc_voltage, ADC_REPLY_ARRAY_SIZE,
	...
	ret = pm8058_htc_config_mpp_and_adc_read(adc.adc_current, ADC_REPLY_ARRAY_SIZE,
			CHANNEL_ADC_BATT_AMON,
	...
	ret = pm8058_htc_config_mpp_and_adc_read(adc.adc_temperature, ADC_REPLY_ARRAY_SIZE,
			CHANNEL_ADC_BATT_AMON,
	...
	ret = pm8058_htc_config_mpp_and_adc_read(adc.adc_battid, ADC_REPLY_ARRAY_SIZE,
			CHANNEL_ADC_BATT_AMON,
	...
	pr_debug("%s , vref:%d, battid_adc:%d, battid:%d\n", __func__,
			vref, battid_adc, battid_adc * 1000 / vref);

	if (ret)
		goto get_adc_failed;

	memcpy(&htc_batt_info.adc_data, &adc, sizeof(struct battery_adc_reply));
...
static void batt_force_shutdown_for_batt_protect (void)
{
	...
	if (htc_batt_info.rep.batt_id != PROTECTION_M8_SONY_BATT_ID) {
		pr_debug("%s: Not SONY battery", __func__);
		return;
	}
#endif
	...
	pr_debug("%s: Level = %d and Voltage = %d.",__func__, htc_batt_info.rep.level, batt_vol);
	...
	if (++count >= PROTECTION_COUNT) {
		...
		pr_debug("%s: Set level = 0 because battery voltage low.",__func__);
...
static void batt_level_adjust(unsigned long time_since_last_update_ms)
{
	...
	if (is_battery_discharging(prev_batt_info_rep)) {
		...
		if (is_voltage_critical_low(htc_batt_info.rep.batt_vol)) {
			...
			pr_debug("battery level force decreses %d%% from %d%%"
...
int update_screen_status_by_filenode (void)
{
	...
	if (strcmp( "panel_power_on = ", file_node_data)) {
		if (file_node_data[SCREEN_FILE_NODE_SIZE - 1] == '0') {
			...
			pr_debug("%s: Screen is OFF", __func__);
		} else {
			htc_batt_info.state &= ~STATE_EARLY_SUSPEND;
			pr_debug("%s: Screen is ON", __func__);
		}
	} else {
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case HTC_BATT_IOCTL_CHARGER_CONTROL: {
		...
		pr_debug("do charger control = %u", charger_mode);
	...
	case HTC_BATT_IOCTL_UPDATE_BATT_INFO: {
		...
		pr_debug("ioctl: battery level update: %u", htc_batt_info.rep.level);
	...
	case HTC_BATT_IOCTL_SET_ALARM_TIMER_FLAG: {
		...
		pr_debug("Set alarm timer flag:%u", flag);
		break;
	}
	default:
		pr_err("%s: no matched ioctl cmd", __func__);
		break;
	}
				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		case FB_BLANK_UNBLANK:
			htc_batt_info.state &= ~STATE_EARLY_SUSPEND;
			pr_debug("%s-> display is On", __func__);
			htc_batt_schedule_batt_info_update();
			break;
		...
		case FB_BLANK_NORMAL:
			htc_batt_info.state |= STATE_EARLY_SUSPEND;
			pr_debug("%s-> display is Off", __func__);
			htc_batt_schedule_batt_info_update();
			break;
		}
	}
		...
		unsigned long event, void *data)
{
	...
	pr_debug("%s: save consistent data", __func__);
...
static int htc_battery_probe(struct platform_device *pdev)
{
	...
	if (!htc_battery_core_ptr) {
		pr_err("%s: kmalloc failed for htc_battery_core_ptr.", __func__);
		return -ENOMEM;
	}
	...
	if (pdata->gpio_mbat_in_trigger_level == MBAT_IN_HIGH_TRIGGER)
		rc = request_irq(pdata->gpio_mbat_in, mbat_int_handler,
				IRQF_TRIGGER_HIGH, "mbat_in", pdata);
	else if (pdata->gpio_mbat_in_trigger_level == MBAT_IN_LOW_TRIGGER)
		rc = request_irq(pdata->gpio_mbat_in, mbat_int_handler,
				IRQF_TRIGGER_LOW, "mbat_in", pdata);
	if (rc)
		pr_err("request mbat_in irq failed!");
	else
	...
	if (rc) {
		pr_err("Unable to register misc device %d", MISC_DYNAMIC_MINOR);
		goto fail;
	}
