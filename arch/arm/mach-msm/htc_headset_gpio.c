...
static void hs_gpio_uart_set(int value)
{
	...
	if (value != headset_uart_enable) {
		...
		pr_debug("%s headset uart", value? "enable": "disable");
...
static irqreturn_t detect_irq_handler(int irq, void *dev_id)
{
	...
	pr_debug("gpio2 = %d (%d retries)", gpio2, (10-retry_limit));
...
static int hs_gpio_request_output(unsigned int gpio, const char *name, int value)
{
	...
	ret = gpio_request(gpio, name);
	if (ret < 0)
		pr_debug("GPIO Already Requested");

	ret = gpio_direction_output(gpio, value);
	if (ret < 0) {
		pr_err("gpio_direction_output(gpio);");
		gpio_free(gpio);
		return ret;
	}
				     ...
				     struct htc_headset_gpio_platform_data *pdata)
{
	...
	if (pdata->hpin_gpio) {
		pr_debug("hpin-gpio:%d", pdata->hpin_gpio);
	} else
		pr_err("hpin-gpio parser err");
	return 0;
}
#else
static int htc_headset_gpio_parse_dt(struct device *dev,
...
static int htc_headset_gpio_probe(struct platform_device *pdev)
{
	...
	if (!hi) {
		pr_err("Failed to allocate memory for headset info");
		return -ENOMEM;
	}
	...
	if (hi->pdata.uart_gpio) {
		...
		if (ret < 0)
			pr_err("Fail to request GPIO UART(0x%x)", ret);
	}
	...
	if (hi->pdata.hpin_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request GPIO HPIN IRQ (0x%X)", ret);
	...
	if (hi->pdata.key_gpio) {
		...
		if (ret < 0) {
			pr_err("Failed to request GPIO button IRQ (0x%X)", ret);
