...
inline int htc_battery_cell_find_and_set_id_auto(int id_raw)
{
	...
	if (!pcell) {
		pr_err("cell pointer is NULL so unknown ID is return.\n");
	...
	if (cur_cell) {
		...
		pr_warn("cur_cell is initiated by %s\n", __func__);
		cur_cell = pcell;
		return pcell->id;
	}
	pr_debug("battery cell id %d -> %d\n",
...
static int __init check_dq_setup(char *str)
{
	if (!strcmp(str, "PASS")) {
		hv_authenticated = 1;
		pr_info("HV authentication passed.\n");
	} else {
		hv_authenticated = 0;
		pr_info("HV authentication failed.\n");
	}
	return 0; 
}
...
static int htc_batt_cell_probe(struct platform_device *pdev)
{
	...
	if (node) {
		...
		for_each_child_of_node(last_node, node) {
			rc = of_property_read_u32(node, "htc,capacity",
			...
			rc = of_property_read_u32(node, "htc,batt_id",
			...
			rc = of_property_read_u32(node, "htc,id_raw_min",
				&id_raw_min);
			if (rc) {
			...
			rc = of_property_read_u32(node, "htc,id_raw_max",
				&id_raw_max);
			if (rc) {
			...
			rc = of_property_read_u32(node, "htc,type",
			...
			rc = of_property_read_string(node, "htc,model_name",
