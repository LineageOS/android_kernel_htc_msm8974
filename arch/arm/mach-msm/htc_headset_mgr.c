...
{
	if (hi->is_ext_insert) {
		pr_debug("Start MIC status polling (%d)", count);
...
void button_pressed(int type)
{
	...
	pr_debug("%s (%d) pressed", key_name, type);
...
void button_released(int type)
{
	...
	get_key_name(type, key_name);
	pr_debug("%s (%d) released", key_name, type);
...
void headset_button_event(int is_press, int type)
{
	if (hi->hs_35mm_type == HEADSET_UNPLUG &&
		...
		pr_debug("IGNORE key %d (HEADSET_UNPLUG)", type);
		return;
	}

	if (!hs_hpin_stable()) {
		pr_debug("IGNORE key %d (Unstable HPIN)", type);
		return;
	}

	if (!get_mic_state()) {
		pr_debug("IGNORE key %d (Not support MIC)", type);
		return;
	}
	...
	if (hi->driver_one_wire_exist && hi->one_wire_mode == 0) {
		...
		if (adc > 915) {
			...
			if (hs_mgr_notifier.hs_1wire_init() == 0) {
				...
				pr_debug("old_state = 0x%x, new_state = 0x%x", old_state, new_state);
...
#endif
		switch_set_state(&hi->sdev_h2w, new_state);
		pr_debug("Sent uevent 0x%x ==> 0x%x", old_state, new_state);
		hpin_report++;
	} else
...
static void debug_work_func(struct work_struct *work)
{
	...
	while (hi->debug_flag & DEBUG_FLAG_ADC) {
		...
		if (hs_mgr_notifier.remote_adc)
			hs_mgr_notifier.remote_adc(&adc);
		pr_debug("Debug Flag %d, HP_DET %d, ADC %d", flag,
		       hpin_gpio, adc);
		msleep(HS_DELAY_SEC);
	}
}
...
static void remove_detect_work_func(struct work_struct *work)
{
	...
	if (hi->remove_count  >= DEBOUNCE_COUNT) {
		...
		pr_debug("Detecting debounce initialized! %d", hi->keep_mic_on);
	} else
		set_35mm_hw_state(0);
#else
	set_35mm_hw_state(0);
...
static void insert_detect_work_func(struct work_struct *work)
{
	...
	case HEADSET_MIC:
		...
		pr_debug("!!!!!!!!!!!!!!!! IMP STATE %d", imp_state);
	...
	hi->hs_35mm_type = mic;
	pr_debug("Send uevent for state change, %d => %d", old_state, new_state);
	switch_set_state(&hi->sdev_h2w, new_state);
	hpin_report++;
...
{
	int ret = 0;
	pr_debug("Headset status++%d++ %d", intr_id,insert);
	...
	if (hi->is_ext_insert) {
		ret = queue_delayed_work(detect_wq, &insert_detect_work,
				   HS_JIFFIES_INSERT);
		pr_debug("queue insert work, ret = %d", ret);
	}
	else {
		...
		pr_debug("queue remove work, ret = %d", ret);
	}
...
int hs_notify_key_event(int key_code)
{
	...
	if (hi->hs_35mm_type == HEADSET_UNKNOWN_MIC ||
		...
		pr_debug("IGNORE key %d (Unstable HPIN)", key_code);
		return 1;
...
static void proc_long_press(void)
{
	if (hi->key_code_1wire[hi->key_code_1wire_index - 1] == HS_MGR_2X_KEY_MEDIA) {	
		pr_debug("long press key found, replace key[%d] = %d ==> %d", hi->key_code_1wire_index - 1,
...
static void button_1wire_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < hi->key_code_1wire_index; i++) {
		pr_debug("1wire key [%d] = %d", i, hi->key_code_1wire[i]);
...
{
	pr_debug("Headset %d, Key %d, Press %d", headset_type, key_code, press);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = (count - 2); i >= 0; i--) {
		pr_debug("buf[%d] = %d, ten_base = %ld", i, *(buf + i) - 48, ten_base);
...
static void htc_headset_mgr_late_resume(struct early_suspend *h)
{
	...
	if (hi->quick_boot_status) {
		...
		pr_debug("Resend quick boot U-Event (state = %d)", state);
		switch_set_state(&hi->sdev_h2w, state);
...
static void headset_power(int enable)
{
	pr_debug("%s_set MIC bias:%s", __func__, enable ? "ON" : "OFF");
...
static void headset_init(void)
{
	...
	pr_debug("%s-Init gpio[%d %d] alt:%d", __func__, htc_headset_mgr_data.tx_gpio,
	...
	for (i = 0; i < ARRAY_SIZE(hs_onewire_gpio); i++) {
		...
		if (ret < 0)
			pr_err("%s config error gpio_idx(%d)",__func__, i);
	}
}
...
static int headset_mgr_parser_dt(struct htc_hs_mgr_data *mgr)
{
	...
	for (i = 0; i < ARRAY_SIZE(parser_st); i++) {
		...
		if (!gpio_is_valid(gpio[i])) {
			pr_err("DT_mgr %sparser fail, ret = %d",parser_st[i], gpio[i]);
			goto parser_fail;
		}
	}
	...
	if (ret < 0) {		
		pr_err("1wire_alt parser err, ret=%d", ret);
		goto parser_fail;
	}

	pr_debug("DT:tx[%d],rx[%d],alt[%d],bias[%d],oe[%d]",gpio[0], gpio[1],
	...
	if (ret < 0) {
		pr_err("mgr,rx_gpio_pulldn parser none, ret=%d", ret);
		htc_headset_mgr_data.rx_gpio_pulldn = 0;
	}
	pr_debug("DT:Headset rx_gpio_pulldn=%d",htc_headset_mgr_data.rx_gpio_pulldn);

	ret = of_property_read_u32(dt, "mgr,hs_typenum", (u32*)&htc_headset_mgr_data.headset_config_num);
	if (ret < 0) {
		pr_err("mgr,hs_typenum parser err, ret=%d", ret);
		goto parser_fail;
	}
	pr_debug("DT:Headset type number = %d",htc_headset_mgr_data.headset_config_num);
