...
static struct file *openFile(char *path,int flag,int mode)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pr_debug("Open: fp count = %d", ++fp_count);
	fp=filp_open(path, flag, mode);
	set_fs(old_fs);
	if (!fp)
		return NULL;
	if (IS_ERR(fp)) {
		pr_debug("File Open Error:%s",path);
		fp_count--;
	...
	if(!fp->f_op) {
		pr_debug("File Operation Method Error!!");
		fp_count--;
		mutex_unlock(&hi->mutex_lock);
		return NULL;
	}
...
int closeFile(struct file *fp)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pr_debug("Close: fp count = %d", --fp_count);
	filp_close(fp,NULL);
	set_fs(old_fs);
	mutex_unlock(&hi->mutex_lock);
...
static void onewire_init_work_func(struct work_struct *work)
{
	pr_debug("Open %s", hi->pdata.onewire_tty_dev);
	fp = openFile(hi->pdata.onewire_tty_dev,O_RDWR|O_NONBLOCK,0660);
	if (fp != NULL) {
		if (!fp->private_data)
			pr_debug("No private data");
		else
			pr_debug("Private data exist");

		closeFile(fp);
		return;
	} else
		pr_err("%s, openFile is NULL pointer\n", __func__);
}
...
static int hs_read_aid(void)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		pr_debug("[1wire]read_count = %d", read_count);
		if (read_count > 0) {
			for (i = 0; i < read_count; i++) {
				pr_debug("[1wire]in_buf[%d] = 0x%x", i, in_buf[i]);
...
static int hs_1wire_open(void)
{
	...
	pr_debug("[1-wire]hs_1wire_read_key");
	if (!ret) {
		pr_err("Cancel fileclose_work failed, ret = %d", ret);
		fp = openFile(hi->pdata.onewire_tty_dev,O_RDWR|O_NONBLOCK,0660);
	}
...
static int hs_1wire_read_key(void)
{
	...
	for (retry = 0; retry < 3;retry++) {
		...
		pr_debug("[1wire]key read_count = %d", read_count);
		if (read_count > 0) {
			for (i = 0; i < read_count; i++) {
				pr_debug("[1wire]key_code[%d] = 0x%x", i, key_code[i]);
...
//			pr_debug("Polling remote_press low");
//		}
//	}
	pr_debug("Send 0x00 0xF5");
	usleep(300);
	if (hi->pdata.tx_level_shift_en)
		gpio_set_value_cansleep(hi->pdata.tx_level_shift_en, 1);
	pr_debug("Disable level shift");
	hr_msleep(22);
	if (hs_read_aid() == 0) {
		pr_debug("[1-wire]Valid AID received, enter 1-wire mode");
		if (hi->pdata.tx_level_shift_en)
			gpio_set_value_cansleep(hi->pdata.tx_level_shift_en, 1);
		closeFile(fp);
		return 0;
...
static int hs_1wire_report_type(char **string)
{
	...
	hi->aid &= 0x7f;
	pr_debug("[1wire]AID = 0x%x", hi->aid);
	if (hi->aid > type_num || hi->aid < 1) {
		*string = "1wire_unknown";
		return 14;
	}else {
		*string = hs_type[hi->aid - 1];
		pr_debug("Report %s type, size %d", *string, sizeof(hs_type[hi->aid -1]));
...
{
	pr_debug("Set gpio[%d] = %d", hi->pdata.uart_tx, enable);
...
static int htc_headset_1wire_probe(struct platform_device *pdev)
{
	...
	pr_debug("1wire tty device %s", hi->pdata.onewire_tty_dev);
	onewire_wq = create_workqueue("ONEWIRE_WQ");
	if (onewire_wq  == NULL) {
		pr_err("Failed to create onewire workqueue");
		kfree(hi);
		kfree(pdata);
		kfree(dtinfo);
		return 0;
