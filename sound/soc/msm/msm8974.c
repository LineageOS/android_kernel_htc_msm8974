...
static char __maybe_unused *msm_prim_auxpcm_gpio_name[][2] = {
...
static char __maybe_unused *msm_sec_auxpcm_gpio_name[][2] = {
...
static int __maybe_unused msm8974_liquid_ext_spk_power_amp_init(void)
{
	int ret = 0;
...
static int __maybe_unused msm8974_liquid_init_docking(struct snd_soc_dapm_context *dapm)
{
	int ret = 0;
...
static int __maybe_unused msm8974_liquid_ext_spk_power_amp_on(u32 spk)
{
	int rc;
...
static void __maybe_unused msm8974_fluid_ext_us_amp_on(u32 spk)
{
...
static void __maybe_unused msm8974_fluid_ext_us_amp_off(u32 spk)
{
	if (!gpio_is_valid(ext_ult_lo_amp_gpio)) {
		pr_err("%s: ext_ult_lo_amp_gpio isn't configured\n", __func__);
	} else if (spk & (LO_1_SPK_AMP | LO_3_SPK_AMP)) {
...
static void __maybe_unused msm8974_liquid_ext_spk_power_amp_off(u32 spk)
{
...
static int __maybe_unused msm_prim_auxpcm_startup(struct snd_pcm_substream *substream)
{
...
static void __maybe_unused msm_prim_auxpcm_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
...
static int __maybe_unused msm_sec_auxpcm_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
...
static void __maybe_unused msm_sec_auxpcm_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (err < 0)
		return err;
#ifdef M8_AUDIO
	err = snd_soc_add_codec_controls(codec, htc_amp_switch_control,
...
static int msm8974_configure_quat_mi2s_gpio(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(htc_mi2s_config.gpio); i++) {
		...
		ret = gpio_request(htc_mi2s_config.gpio[i].gpio_no,
...
*/
		pr_info("%s: gpio = %d, gpio name = %s, ret = %d\n", __func__,
			htc_mi2s_config.gpio[i].gpio_no, htc_mi2s_config.gpio[i].gpio_name, ret);
...
static int __maybe_unused msm8974_dtparse_auxpcm(struct platform_device *pdev,
...
static int __maybe_unused msm8974_prepare_us_euro(struct snd_soc_card *card)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int ret;
